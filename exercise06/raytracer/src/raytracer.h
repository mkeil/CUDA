...
bool intersectNew(const ray& r, const triangle& tri, point& intersection);
...
float disToCam (const point& p, const camera& c);
float disToLight (const point& p, point light);
...
bool pointInShadow (point* light, const point& intersecPoint,triangle* triPnt, int nrTri, const triangle& ignoreTriangle);
